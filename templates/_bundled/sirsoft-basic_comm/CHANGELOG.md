...
- 홈 데이터 소스를 특정 게시판 slug가 필요 없는 공개 게시판 API로 분리하여 원본 사이트의 `home--strapi` 요청 및 404 재발 방지
- 게시판이 없는 설치 환경에서도 통계는 0, 목록은 빈 배열 fallback으로 정상 렌더링

## [1.0.0] - 2026-08-23

### Changed

- `sirsoft-comm` 기반 템플릿을 독립 identifier `sirsoft-basic_comm`로 전환
- 템플릿 소유자 및 패키지 작성자 metadata를 Glitter.kr로 정비
- 외부 저장소 상태와 분리된 bundled update source를 초기 update metadata로 채택
- 에러 레이아웃 metadata를 현행 installer가 사용하는 `errors/*` 경로와 일치시킴
- 홈 환영 문구를 hardcoded text 대신 기존 translation key로 전환

## [1.0.0-beta.4] - 2026-05-14

### Changed

- 홈 화면 초기 로딩 데이터를 단일 API로 가져오도록 변경하여 첫 렌더링 요청 수 감소
