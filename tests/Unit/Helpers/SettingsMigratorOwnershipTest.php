 ...
 *
 * 본 테스트는 `FilePermissionHelper::syncOwnershipFromParent` public wrapper를 통해
 * 외부(SettingsMigrator)가 내부 protected primitive를 노출하지 않고 호출 가능한지 검증.
 */
class SettingsMigratorOwnershipTest extends TestCase
{
     ...
     * @return void
     */
    public function test_parent_ownership_sync_wrapper_is_public(): void
    {
        $reflection = new \ReflectionMethod(FilePermissionHelper::class, 'syncOwnershipFromParent');
        $primitive = new \ReflectionMethod(FilePermissionHelper::class, 'inheritOwnershipFromParent');

        $this->assertTrue(
            $reflection->isPublic(),
            'FilePermissionHelper::syncOwnershipFromParent 가 public 이어야 외부 호출 가능 (SettingsMigrator 등)',
        );
        $this->assertTrue($reflection->isStatic(), 'static 메서드여야 함');
        $this->assertTrue($primitive->isProtected(), '내부 ownership primitive는 protected여야 함');
    }
    ...
    public function test_inherit_ownership_from_parent_idempotent_for_self_owner(): void
    {
        ...
        FilePermissionHelper::syncOwnershipFromParent($child);
    ...
    public function test_settings_migrator_write_json_file_invokes_inherit_ownership(): void
    {
        ...
        $this->assertStringContainsString(
            'syncOwnershipFromParent',
            $methodBody,
            'SettingsMigrator::writeJsonFile 가 sudo update 시 root 로 만든 *.json 의 owner 를 부모로 상속해야 함 (public wrapper 호출)',
        );
    }
}
