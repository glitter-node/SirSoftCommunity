...
{
    public function test_parent_ownership_sync_exposes_wrapper_without_exposing_internal_primitive(): void
    {
        $wrapper = new \ReflectionMethod(FilePermissionHelper::class, 'syncOwnershipFromParent');
        $primitive = new \ReflectionMethod(FilePermissionHelper::class, 'inheritOwnershipFromParent');

        $this->assertTrue($wrapper->isPublic());
        $this->assertTrue($wrapper->isStatic());
        $this->assertTrue($primitive->isProtected());
    }
