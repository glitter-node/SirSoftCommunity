...
use Modules\Sirsoft\Board\Http\Requests\User\HomeBootstrapRequest;
...
class BoardController extends PublicBaseController
{
    ...
    /**
     * 공개 홈 화면에 필요한 게시판 데이터를 한 요청으로 반환합니다.
     */
    public function homeBootstrap(HomeBootstrapRequest $request): JsonResponse
    {
        $homeBoardPostsLimit = $request->homeBoardPostsLimit();
        $homeBoards = $this->boardService->getReadableActiveBoards()->map(function ($board) use ($homeBoardPostsLimit) {
            $data = (new BoardResource($board))->toListArray();
            $data['recent_posts'] = $homeBoardPostsLimit > 0
                ? $this->boardService->getBoardRecentPostsById($board->id, $homeBoardPostsLimit)
                : [];
        ...
        return $this->success('common.success', [
            'stats' => $this->boardService->getCachedStats(),
            'recent_posts' => $this->boardService->getCachedRecentPosts($request->recentPostsLimit()),
            'popular_boards' => $this->boardService->getCachedPopularBoards($request->popularBoardsLimit()),
            'home_boards' => $homeBoards,
        ]);
    }
