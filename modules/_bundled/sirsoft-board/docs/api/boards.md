...
### GET /api/modules/sirsoft-board/boards/home-bootstrap

공개 홈 화면에서 사용하는 통합 초기 데이터 API입니다. 인증은 선택 사항이며 현재 사용자가
읽을 수 있는 게시판만 `popular_boards`와 `home_boards`에 포함됩니다.

| 쿼리 | 기본값 | 범위 | 설명 |
| --- | ---: | ---: | --- |
| `recent_posts_limit` | 5 | 0~20 | 전체 최근 게시글 수 |
| `popular_boards_limit` | 4 | 0~20 | 인기 게시판 수 |
| `home_board_posts_limit` | 3 | 0~10 | 홈 게시판별 최근 게시글 수 |

응답 `data`는 `stats`, `recent_posts`, `popular_boards`, `home_boards`를 포함하며 잘못된
범위는 422로 거부됩니다.
