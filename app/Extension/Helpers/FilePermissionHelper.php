...
class FilePermissionHelper
{
    ...
    /**
     * 대상 경로의 소유자·그룹을 부모 디렉토리와 동기화합니다.
     *
     * 외부 helper와 upgrade step이 사용하는 공개 진입점입니다. ACL을 변경하거나 초기화하지
     * 않으며, 기존 내부 primitive와 동일하게 POSIX owner/group만 best-effort로 정합화합니다.
     *
     * @param  string  $path  동기화할 파일 또는 디렉토리
     */
    public static function syncOwnershipFromParent(string $path): void
    {
        static::inheritOwnershipFromParent($path);
    }

    /**
     * 단일 경로에만 POSIX 그룹 쓰기 비트를 보장합니다.
     *
     * 하위 트리를 순회하지 않고 ACL을 생성·삭제·초기화하지 않습니다. 업데이트용 부모
     * 디렉터리처럼 현재 그룹이 쓰기 가능해야 하는 정확한 경로에만 사용합니다.
     */
    public static function ensureGroupWritable(string $path): bool
    {
        $permissions = @fileperms($path);
        if ($permissions === false || ($permissions & 0020) === 0020) {
            return false;
        }

        return @chmod($path, $permissions | 0020);
    }
