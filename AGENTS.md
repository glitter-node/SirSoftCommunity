...
## Bundled Extension Workflow Rule (CRITICAL)

All extension source changes must be made only in bundled source directories.

### Editable source directories

- templates/_bundled/{template_id}/
- modules/_bundled/{module_id}/
- plugins/_bundled/{plugin_id}/

### Forbidden direct edit targets
...
Installed or active extension directories are deployment outputs, not source of truth.

After changing bundled sources, agents must apply changes through the proper update/install/activate lifecycle.

Direct edits to installed or active extension directories are not allowed.

---

## Extension Apply Loop Rule (CRITICAL)

After bundled source changes, agents must run the appropriate lifecycle command sequence.

### Required loop

1. Modify bundled source only
2. Run relevant tests
3. Run build if frontend assets changed
4. Run extension update/install/activate command as appropriate
5. Verify the active output
6. Inspect git diff and report changed files

Agents must not mark work as complete until the active extension reflects bundled source changes.

---

## Database Safety Rule (CRITICAL)

Before any database-related change, agents must inspect the current schema first.
...
- enum/status values
- engine/collation differences
- MariaDB compatibility

Do not assume schema from memory or documentation alone.

### Database Metadata Compatibility

When inspecting database metadata (e.g. information_schema.columns), agents must never assume that metadata is returned as lowercase stdClass properties.

Agents must:

- support both stdClass and associative array results
- normalize metadata keys to lowercase before use
- validate required fields with array_key_exists()
- distinguish missing fields from NULL values
- throw clear exceptions when required metadata is unavailable
...
Every compatibility bug fix must include a regression test covering the affected metadata format.

---

## Verification Rule (CRITICAL)

Every change must be verified against the actual runtime model.

### Minimum verification

- git status
- git diff
- relevant PHPUnit or Vitest tests
- npm run build when frontend assets changed
- /usr/local/bin/php83 artisan command when backend/runtime validation is needed

### Frontend verification

Frontend changes must be verified against built assets, not a dev server.

### Layout verification

Layout JSON changes must be validated with layout rendering tests when applicable.

### Production-like auth verification

Do not verify production registration by creating persistent real users.

- Use validation-only payloads, duplicate payloads, test database, or rollback-capable scripts.
- If a persistent registration is accidentally created, inspect related schema first and remove only clearly test-created rows plus directly related auth rows.

Completion without verification is not allowed.

---

## Codex Prompt Rule (CRITICAL)

All implementation plans given to Codex must explicitly include:

- Use Codex CLI
- Ask follow-up questions in Korean
- Do not use Docker
- Do not use npm run dev
- Do not use php or composer directly
- Use /usr/local/bin/php83
- Edit bundled source directories only
- Do not directly edit installed or active extension directories
- Preserve i18n layout rules
- Run the update/install/activate lifecycle after bundled changes
- Run tests and report git diff

---

## Runtime Environment Assumption Rule (CRITICAL)
...
All runtime configuration must be verified from the current project state before use.

Do not infer runtime values from previous conversations alone.

---

## Source Of Truth Rule (CRITICAL)

Bundled source directories are the only source of truth.

Agents must never:

- treat installed extension directories as editable source
- patch active runtime directories directly
- patch generated assets manually
- bypass extension lifecycle flows

All fixes must originate from bundled sources first.

---

## Build Watch Rule (CRITICAL)

When frontend work is active:

- npm run build:watch should remain continuously running

Agents must not assume assets are updated until build output regeneration is verified.
...
agents must verify whether cache invalidation is required.
...
Agents must not assume runtime changes are visible without cache validation.

---

## ACL / Permission Safety Rule (CRITICAL)

This project runs on NAS environments with ACL-based permission inheritance.

Agents must not:

- overwrite Synology ACL structures blindly
- remove inherited ACL behavior
- assume chmod/chown alone is sufficient
- replace ACL handling with generic Linux-only permission fixes

Permission-related changes must preserve ACL inheritance behavior.

---

## Failure Handling Rule (CRITICAL)

Agents must never declare completion when:

- tests fail
- build output is stale
- active extension output is outdated
- runtime verification was skipped
- only bundled source edits were completed

If verification cannot be completed, agents must explicitly state what remains unverified.

---

## Layout Engine Rule (CRITICAL)

Agents must never invent unsupported layout JSON syntax.
...
agents must verify support from:

- existing engine implementation
- existing production layouts
- official documentation

Unsupported or assumed syntax is forbidden.

---

## Test First Rule (CRITICAL)

For bug fixes:

1. Reproduce the issue first
2. Create or identify a failing regression test
3. Confirm the failure
4. Apply the fix
5. Verify tests pass

Agents must not patch blindly without reproduction or verification.

---

## Generated Asset Rule (CRITICAL)

Generated files are not editable source.

Agents must never directly modify:

- public/build/*
- compiled assets
- generated manifests
- cache-generated runtime files

Changes must originate from source files and flow through the build system.

---

## Reporting Rule (CRITICAL)

After completing work, agents must report:
...
Agents must not provide vague completion summaries.

---

## Manifest Version Rule (CRITICAL)

When extension source behavior changes:

- template.json
- module.json
- plugin.json

version fields must be reviewed and updated appropriately.

If version changes are required but skipped, extension update detection may fail.

---

## Node Binary Verification Rule (CRITICAL)

Before declaring frontend verification complete, agents must verify that native Node executables are runnable.
...
- executable bit
- Synology ACL inheritance
- actual execution success

### Failure policy

Build verification is invalid if native binaries cannot execute.

Agents must not:

- silently skip native binary execution failures
- assume npm install completed successfully implies executable integrity
- apply recursive chmod fixes without ACL verification

### Minimum inspection commands

```bash
ls -la node_modules/.bin/vite
readlink node_modules/.bin/vite
ls -la node_modules/@esbuild/*/bin/esbuild
synoacltool -get node_modules
```

---

## Package Script Consistency Rule (CRITICAL)

If AGENTS.md references required npm scripts,
agents must verify those scripts actually exist in package.json.
...
- scripts.test
- scripts.test:run
- extension-specific build scripts if referenced

### Failure policy

Documentation assumptions without executable scripts are invalid.

Agents must not:

- reference nonexistent npm scripts
- assume build/watch scripts exist
- declare build workflows complete without package.json verification
...
## Runtime Build Freshness Rule (CRITICAL)

Agents must verify that generated frontend assets are fresh and synchronized with current bundled sources.

### Required validation

Agents must verify:

- public/build assets are freshly generated
- manifest timestamps are newer than source modifications
- active template assets match current bundled sources
- stale build artifacts are not being reused

### Failure policy

Stale generated assets invalidate release verification.

Agents must not:

- assume previous build artifacts are valid
- verify UI against stale assets
- skip manifest freshness inspection

### Minimum inspection targets

- public/build/manifest.json
- public/build/core/*
- templates/{active}/assets/*
- bundled source modification timestamps

### Minimum inspection commands

```bash
find public/build -type f | head
stat public/build/manifest.json
find templates/_bundled -type f -newer public/build/manifest.json
```

---

## Production Exposure Rule (CRITICAL)

Production environments must never expose development or debugging surfaces.

### Forbidden production exposure

- debug routes
- devtools routes
- playground routes
- temporary diagnostics
- internal boost/debug endpoints
- test-only APIs
- local-only tooling routes

### Required verification

Agents must verify route exposure using:

```bash
/usr/local/bin/php83 artisan route:list
```

### Failure policy

Release verification fails if public production environments expose debug tooling.

Agents must not:

- rely only on APP_ENV checks
- assume routes are hidden
- ignore debug routes because authentication exists

### Required validation targets

- routes/web.php
- routes/devtools.php
- extension route providers
- conditional route registration logic

---

## Demo Content Leakage Rule (CRITICAL)

Production environments must not expose development/demo content unintentionally.
...
- template fallback images
- public demo URL accessibility

### Failure policy

Public demo leakage invalidates production readiness verification.

Agents must not:

- assume fallback assets are private
- keep demo paths in production layouts
- reuse development branding in public templates
...
```bash
grep -R "demo" templates/_bundled
grep -R "placeholder" templates/_bundled
grep -R "lorem" modules/_bundled
curl -I https://example.com/images/demo/sample.jpg
```
